...
class Search extends React.Component {
  ...
  componentDidMount() {
    ...
    } else if (this.props.location.search_obj) {
      this.setState(
        {
          search: this.props.location.search_obj,
        },
        () => {
          this.fetchAssets();
        }
      ...
      this.setState(
        ...
        () => {
          ...
          if (AuthStore.token !== undefined) {
            AuthStore.myCollection(AuthStore.token).then((res) => {
              this.setState({
                assets: res.data,
                collectionAsset: res.data,
                page: 1,
                last_page: 1,
                isLoading: false,
                pageCount: 0,
  ...
  selectCategory = (selectedCategory) => {
    let { category, isLoading, myCollection, collectionAsset } = this.state;

    if (!isLoading) {
      if (myCollection) {
        let newArray = [];
        collectionAsset.forEach((obj) => {
      ...
      <SearchPage>
        <LeftSection>
          <Sticy>
            ...
            {AuthStore.token !== undefined && AuthStore.token !== false && (
              <ButtonSection>
                {myCollection ? (
                  <FilterForButton
                    onClick={() => this.myCollectionFalse()}
                    disabled={isLoading}
                  >
                    My Collection
                  </FilterForButton>
